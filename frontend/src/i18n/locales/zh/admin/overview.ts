export default {
    ...
    users: {
      ...
      platformQuota: {
        ...
        usdSectionTitle: '平台 USD 消费限额',
        usdSectionDescription: '这里限制各上游平台按美元计费的日 / 周 / 月消费，与 Codex 官方 5h / 7d 窗口额度相互独立。',
        columns: {
          platform: '平台',
          daily: '日 (USD)',
          weekly: '周 (USD)',
        ...
        cancel: '取消',
        clearAll: '全部设为不限',
        clearAllConfirm: '确认将全部平台的日 / 周 / 月限额设为“不限”？这是高风险批量变更，保存后会移除所有平台 USD 限制。',
        disableConfirm: '确认禁用 {platform} 的{window}额度？保存后该平台在此窗口将无法使用。',
        discardConfirm: '存在未保存的额度变更，确认放弃并关闭吗？',
        unsavedIndicator: '有未保存变更',
        states: {
          title: '三态说明：',
          explanation: '不限 = null；禁用 = 0；限额 = 正数。三种状态会按原值提交，不会互相转换。',
          unlimited: '不限',
          disabled: '禁用',
          limited: '限额',
        },
      ...
      windowQuota: {
        title: '窗口百分比配额（Codex）',
        subtitle: '每人在官方 5h / 7d 窗口的占用上限（默认各 23%）',
        officialDescription: '这里配置 Codex 官方窗口利用率的成员份额，不是美元消费限额；5h / 7d 会跟随官方窗口独立重置。',
        ceilingLabel: '窗口总额上限（所有用户之和）：',
        save: '保存',
        empty: '暂无数据——产生用量后自动建行，默认 23% 已生效。',
        hint: '归因已用% 来自官方利用率分摊，重置跟随官方窗口刷新。按账号按窗口设置基础 limit%。',
        account: '账号 #{id}',
        window5h: '5h',
        window7d: '7d（周）',
        official: '官方',
        ...
        columns: {
          ...
          effectiveLimit: '有效上限',
          remaining: '剩余',
          officialAttribution: '官方 / 未归因',
          reset: '重置时间',
        },
        saveSuccess: '已保存',
        saveFailed: '保存失败',
        invalidLimit: 'limit 必须是 0–100 之间的数字',
        disableConfirm: '确认把账号 #{account} 的 {window} 额度设为 0 吗？该用户将无法使用此官方窗口。',
    ...
    windowQuotaConfig: {
      ...
      invalidCeiling: '总额上限必须是 1–100 之间的数字',
      note: '“新记录默认人数”只用于未来自动创建的额度行，保存不会改写任何已有成员行；需要调整现有成员时，请在总览中使用安全均分。窗口总额上限保存后立即生效。',
    },
    ...
    windowQuotaOverview: {
      ...
      window7d: '7d',
      donor: '{window} 捐{pct}%',
      donorTip: '已将部分份额捐入救急池（5h / 7d 各自独立）',
      attributed: '归因',
      baseLimit: '基础上限',
      effectiveLimit: '有效上限',
      remaining: '剩余',
      ...
      ceiling: '安全上限',
      overallocated: '已超配',
      overallocatedWarning: '当前已配置总和超过安全上限，请尽快调整或按当前成员安全均分。',
      rebalance: '按当前成员安全均分',
      previewTitle: '安全均分预览',
      previewMembers: '将按当前 {count} 名成员重写此账号的 5h / 7d limit。',
      previewValue: '每人 {pct}%',
      previewHint: '均分值按各窗口安全上限 ÷ 当前成员数计算，会覆盖现有成员的手动 limit。',
      confirmRebalance: '确认均分',
      rebalanceConfirm: '确认按账号 #{id} 的当前成员执行安全均分？现有成员的 5h / 7d limit 将被覆盖。',
      rebalanceSuccess: '安全均分完成，已刷新总览',
      rebalanceFailed: '安全均分失败',
      manageMembers: '指定拼车成员',
      memberEditorTitle: '指定成员与自定义额度',
      memberEditorHint: '勾选此账号的拼车成员，并为每人分别填写 5h / 7d 基础上限；取消勾选会将该用户移出此拼车账号。',
      memberSearch: '搜索用户名、邮箱或用户 ID',
      memberColumn: '成员',
      member5hLimit: '5h 上限',
      member7dLimit: '7d 上限',
      memberAllocation: '{window} 已分配 {total}% / 安全上限 {ceiling}%',
      memberLimitHint: '额度单位为官方窗口百分比，0 表示该成员在对应窗口不可用；两个窗口分别计算。',
      invalidMemberLimit: '用户 #{user} 的 {window} 上限必须是 0–100 之间的数字',
      memberCeilingExceeded: '{window} 已分配 {total}%，超过安全上限 {ceiling}%',
      saveMembers: '保存成员与额度',
      savingMembers: '保存中...',
      membersConfirm: '确认将账号 #{id} 更新为当前 {count} 名成员，并保存额度（5h 共 {total5h}%，7d 共 {total7d}%）？',
      membersSaved: '拼车成员和自定义额度已保存',
      membersSaveFailed: '保存拼车成员失败',
      memberLimitsSaveFailed: '成员已同步，但自定义额度未能全部保存；已刷新服务器当前实际状态',
      loadMembersFailed: '加载用户列表失败',
    },
