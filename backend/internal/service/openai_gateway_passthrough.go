...
func (s *OpenAIGatewayService) handleOpenAIStreamTerminalAccountSideEffects(
	...
	return s.handleOpenAIStreamTerminalAccountSideEffectsAt(c, account, payload, message, headers, time.Now(), canonicalModel...)
}

func (s *OpenAIGatewayService) handleOpenAIStreamTerminalAccountSideEffectsAt(
	c *gin.Context,
	account *Account,
	payload []byte,
	message string,
	...
	case http.StatusUnauthorized, http.StatusTooManyRequests, 529:
		...
		return statusCode, s.handleOpenAIAccountUpstreamErrorAt(ctx, account, statusCode, accountHeaders, payload, observedAt, model)
	default:
		return statusCode, false
	}
}
