...
func (s *OpenAIGatewayService) handleOpenAIStreamTerminalAccountSideEffects(
	...
	return s.handleOpenAIStreamTerminalAccountSideEffectsAt(c, account, payload, message, headers, time.Now())
}
...
func (s *OpenAIGatewayService) handleOpenAIStreamTerminalAccountSideEffectsAt(
	...
	case http.StatusUnauthorized, http.StatusTooManyRequests, 529:
		...
		return statusCode, s.handleOpenAIAccountUpstreamErrorAt(ctx, account, statusCode, accountHeaders, payload, observedAt)
	default:
		return statusCode, false
	}
}
