...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	if err != nil {
		...
		if errors.As(err, &dialErr) && dialErr != nil && dialErr.StatusCode == http.StatusTooManyRequests {
			s.persistOpenAIWSRateLimitSignal(ctx, account, dialErr.ResponseHeaders, nil, dialErr.ObservedAt, "rate_limit_exceeded", "rate_limit_error", strings.TrimSpace(err.Error()), mappedModel)
		}
...
readLoop:
	for {
		...
		if eventType == "error" {
			...
			s.persistOpenAIWSRateLimitSignal(ctx, account, lease.HandshakeHeaders(), message, lease.HandshakeObservedAt(), errCodeRaw, errTypeRaw, errMsgRaw, mappedModel)
