...
func validateAPIKeyConcurrency(v int) error {
	if v < 0 || int64(v) > 2147483647 {
		return infraerrors.BadRequest("API_KEY_CONCURRENCY_INVALID", "max_concurrency must be an integer between 0 and 2147483647")
	}
	return nil
}
...
func validateCreateAPIKeyRequest(req CreateAPIKeyRequest) error {
	if err := validateAPIKeyConcurrency(req.MaxConcurrency); err != nil {
		return err
	}
