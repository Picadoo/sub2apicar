	...
	if resp.StatusCode >= 400 {
		...
		if s.shouldFailoverUpstreamError(resp.StatusCode) {
			...
			if s.rateLimitService != nil {
				shouldDisable = s.rateLimitService.HandleUpstreamErrorAt(ctx, account, resp.StatusCode, resp.Header, respBody, HTTPUpstreamResponseHeadersObservedAt(resp), mappedModel)
			}
