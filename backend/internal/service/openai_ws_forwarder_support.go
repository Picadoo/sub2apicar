...
func (s *OpenAIGatewayService) performOpenAIWSGeneratePrewarm(
	...
	for {
		...
		if eventType == "error" {
			...
			s.persistOpenAIWSRateLimitSignal(ctx, account, lease.HandshakeHeaders(), message, lease.HandshakeObservedAt(), errCodeRaw, errTypeRaw, errMsgRaw, prewarmModel)
...
func (s *OpenAIGatewayService) handleOpenAIWSTerminalTransientFailureAt(ctx context.Context, account *Account, canonicalModel string, headers http.Header, payload []byte, observedAt time.Time) string {
	...
	s.handleOpenAIWSFailureAccountSideEffectsAt(ctx, account, canonicalModel, headers, payload, observedAt)
	return terminalEvent
}
...
func (s *OpenAIGatewayService) handleOpenAIWSFailureAccountSideEffects(ctx context.Context, account *Account, canonicalModel string, headers http.Header, payload []byte) bool {
	return s.handleOpenAIWSFailureAccountSideEffectsAt(ctx, account, canonicalModel, headers, payload, time.Now())
}

func (s *OpenAIGatewayService) handleOpenAIWSFailureAccountSideEffectsAt(ctx context.Context, account *Account, canonicalModel string, headers http.Header, payload []byte, observedAt time.Time) bool {
	message := extractOpenAISSEErrorMessage(payload)
	status := openAIStreamFailureStatus(payload, message)
	switch status {
	case http.StatusUnauthorized, http.StatusTooManyRequests, 529:
		s.handleOpenAIStreamTerminalAccountSideEffectsAt(nil, account, payload, message, headers, observedAt, canonicalModel)
		return true
...
func (s *OpenAIGatewayService) handleOpenAIWSFailureAccountSideEffectsOnceAt(applied *bool, ctx context.Context, account *Account, canonicalModel string, headers http.Header, payload []byte, observedAt time.Time) bool {
	if applied != nil && *applied {
		return true
	}
	handled := s.handleOpenAIWSFailureAccountSideEffectsAt(ctx, account, canonicalModel, headers, payload, observedAt)
	if applied != nil && handled {
		*applied = true
	}
	return handled
}
...
func (s *OpenAIGatewayService) persistOpenAIWSRateLimitSignal(ctx context.Context, account *Account, headers http.Header, responseBody []byte, observedAt time.Time, codeRaw, errTypeRaw, msgRaw string, canonicalModel ...string) {
	...
	s.handleOpenAIAccountUpstreamErrorAt(ctx, account, http.StatusTooManyRequests, headers, responseBody, observedAt, model)
}
