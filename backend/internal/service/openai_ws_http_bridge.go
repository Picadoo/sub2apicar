	...
	for scanner.Scan() {
		...
		if eventType == "error" || eventType == "response.failed" {
			...
			if account.Platform != PlatformGrok && !failureAccountSideEffectsApplied {
				if eventType == "response.failed" || (!officialOpenAIResponses && shouldFailover && !requestScopedCapacity) {
					failureAccountSideEffectsApplied = s.handleOpenAIWSFailureAccountSideEffectsAt(ctx, account, mappedModel, resp.Header, upstreamMessage, HTTPUpstreamResponseHeadersObservedAt(resp))
				}
			}
		...
		if isOpenAIWSTerminalEvent(eventType) && !bareErrorPending {
			if eventType == "response.failed" {
				...
				upstreamTerminalEvent = s.handleOpenAIWSTerminalTransientFailureAt(ctx, account, mappedModel, resp.Header, upstreamMessage, HTTPUpstreamResponseHeadersObservedAt(resp))
			}
			terminalEventCount++
			firstTokenMsValue := -1
			if firstTokenMs != nil {
