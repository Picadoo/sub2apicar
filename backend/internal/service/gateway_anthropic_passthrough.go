...
func invalidNonStreamingJSONFailoverError(
	...
	if rateLimitService != nil && account != nil {
		if len(requestedModel) > 0 {
			rateLimitService.HandleUpstreamErrorAt(ctx, account, statusCode, resp.Header, body, HTTPUpstreamResponseHeadersObservedAt(resp), requestedModel[0])
		} else {
			rateLimitService.HandleUpstreamErrorAt(ctx, account, statusCode, resp.Header, body, HTTPUpstreamResponseHeadersObservedAt(resp))
		}
	}
