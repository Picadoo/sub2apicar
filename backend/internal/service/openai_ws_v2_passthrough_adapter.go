...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	for {
		...
		if statusCode == http.StatusTooManyRequests {
			s.persistOpenAIWSRateLimitSignal(ctx, account, handshakeHeaders, nil, handshakeObservedAt, "rate_limit_exceeded", "rate_limit_error", strings.TrimSpace(err.Error()), capturedSessionModel)
		...
		Options: openaiwsv2.RelayOptions{
			...
			BeforeWriteClient: func(msgType coderws.MessageType, payload []byte, wroteDownstream bool) error {
				...
				if (eventType == "error" || eventType == "response.failed") && !failureAccountSideEffectsApplied && !isPreOutputRateLimit {
					failureAccountSideEffectsApplied = s.handleOpenAIWSFailureAccountSideEffectsAt(ctx, account, capturedSessionModel, handshakeHeaders, payload, handshakeObservedAt)
				}
				if eventType != "error" {
					return nil
				}
				...
				s.persistOpenAIWSRateLimitSignal(ctx, account, handshakeHeaders, payload, handshakeObservedAt, errCodeRaw, errTypeRaw, errMsgRaw, capturedSessionModel)
				logOpenAIWSV2Passthrough(
