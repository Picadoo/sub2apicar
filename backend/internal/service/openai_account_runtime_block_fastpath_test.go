...
func TestOpenAI429FastPath_UsesObservedResetWhenFallbackDisabled(t *testing.T) {
	for _, tc := range []struct {
		name    string
		delay   time.Duration
		blocked bool
	}{
		{name: "active reset keeps original deadline", delay: 30 * time.Second, blocked: true},
		{name: "expired reset does not reblock", delay: 2 * time.Minute, blocked: false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			settingRepo := newMockSettingRepo()
			settingRepo.data[SettingKeyRateLimit429CooldownSettings] = `{"enabled":false,"cooldown_seconds":12}`
			rateLimits := NewRateLimitService(&oauth429RateLimitRepo{}, nil, &config.Config{}, nil, nil)
			...
			observedAt := time.Now().Add(-tc.delay)

			svc.markOpenAIOAuth429RateLimitedAt(context.Background(), account, headers, nil, observedAt)

			snapshot := svc.peekOpenAIAccountRuntimeBlock(account)
			require.Equal(t, tc.blocked, snapshot.blocked)
			if tc.blocked {
				require.WithinDuration(t, observedAt.Add(time.Minute), snapshot.until, time.Millisecond)
			}
		})
	}
}
...
func TestOpenAIWSErrorEvent_OrdinaryModelIgnoresHandshakeQuotaHeaders(t *testing.T) {
	...
	svc.persistOpenAIWSRateLimitSignal(context.Background(), account, headers, payload, time.Now(), "rate_limit_exceeded", "rate_limit_error", "quota exhausted", "gpt-5.3-codex")
...
func TestOpenAIWSErrorEvent_SparkQuotaUsesHandshakeQuotaHeaders(t *testing.T) {
	...
	svc.persistOpenAIWSRateLimitSignal(context.Background(), account, headers, payload, time.Now(), "rate_limit_exceeded", "rate_limit_error", "quota exhausted", "gpt-5.3-codex-spark")
