	...
	acquireTurnLease = func(turn int, preferred string, forcePreferredConn bool, forceNewConn bool) (*openAIWSConnLease, error) {
		...
		if acquireErr != nil {
			...
			if errors.As(acquireErr, &dialErr) && dialErr != nil && dialErr.StatusCode == http.StatusTooManyRequests {
				s.persistOpenAIWSRateLimitSignal(ctx, account, dialErr.ResponseHeaders, nil, dialErr.ObservedAt, "rate_limit_exceeded", "rate_limit_error", strings.TrimSpace(acquireErr.Error()), canonicalModel)
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string, imageBillingModel string, imageSizeTier string, imageInputSize string, requestedReasoningEffort *string) (*OpenAIForwardResult, error) {
		...
		for {
			...
			if eventType == "error" {
				...
				if openAIWSPayloadTransientStatus(upstreamMessage) != 0 {
					s.handleOpenAIWSFailureAccountSideEffectsOnceAt(&failureAccountSideEffectsApplied, ctx, account, canonicalModel, lease.HandshakeHeaders(), upstreamMessage, lease.HandshakeObservedAt())
				}
				...
				s.persistOpenAIWSRateLimitSignal(ctx, account, lease.HandshakeHeaders(), upstreamMessage, lease.HandshakeObservedAt(), errCodeRaw, errTypeRaw, errMsgRaw, mappedModel)
