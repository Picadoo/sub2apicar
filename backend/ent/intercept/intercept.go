...
// The UserAccountWindowQuotaFunc type is an adapter to allow the use of ordinary function as a Querier.
type UserAccountWindowQuotaFunc func(context.Context, *ent.UserAccountWindowQuotaQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f UserAccountWindowQuotaFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.UserAccountWindowQuotaQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.UserAccountWindowQuotaQuery", q)
}

// The TraverseUserAccountWindowQuota type is an adapter to allow the use of ordinary function as Traverser.
type TraverseUserAccountWindowQuota func(context.Context, *ent.UserAccountWindowQuotaQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseUserAccountWindowQuota) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseUserAccountWindowQuota) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.UserAccountWindowQuotaQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.UserAccountWindowQuotaQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.UserAccountWindowQuotaQuery:
		return &query[*ent.UserAccountWindowQuotaQuery, predicate.UserAccountWindowQuota, useraccountwindowquota.OrderOption]{typ: ent.TypeUserAccountWindowQuota, tq: q}, nil
